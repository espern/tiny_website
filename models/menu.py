...
pages = db((db.page.is_index==False) & ((db.page.is_enabled==True) \
	| (db.page.is_enabled==None))).select(orderby=db.page.rank|db.page.title,
...
nb_booking_requests = db(db.calendar_booking_request.is_confirmed==False).count(
	cache=(cache.ram,60))
management_menu = []
if (WEBSITE_PARAMETERS and WEBSITE_PARAMETERS.show_booking_menu and nb_booking_requests and \
	(auth.has_membership('manager')) or auth.has_membership('booking_manager')):
	management_menu += [(T('Booking requests (%d)', nb_booking_requests),
		 False, URL('calendar','edit_booking_requests'))]
if (WEBSITE_PARAMETERS and WEBSITE_PARAMETERS.show_event_menu and \
	(auth.has_membership('manager')) or auth.has_membership('event_manager')):
	management_menu += [(T('Events calendar'), False, 
		URL('calendar','edit_events_calendar'))]
	management_menu += [(T('Contacts calendar'), False, 
		URL('calendar','edit_contacts_calendar'))]
if auth.has_membership('manager'):
	management_menu += [(T('Website settings'), False, 
